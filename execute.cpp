...
// Propagate known values throughout the state.
void propagate_constants(state& s)
{
    bool changed;
    do {
        changed = false;
        ...
        for(auto& inner: s.pc.ors) {
            if(inner.unsatisfiable) continue;

            std::map<unsigned int, concrete> known_values;

            // find all EQ_EXPRs with concrete values
            for(const auto& term: inner.ands) {
                if(term.op != EQ_EXPR) continue;

                if(term.lhs.is_symbolic() && term.rhs.is_concrete())
                    known_values[term.lhs.get_symbolic().version] 
                        = std::get<concrete>(term.rhs.content);
                
                else if(term.lhs.is_concrete() && term.rhs.is_symbolic())
                    known_values[term.rhs.get_symbolic().version]
                        = std::get<concrete>(term.rhs.content);
            }

            // then substitute known values and evaluate
            for(auto& term: inner.ands) {
                if(term.is_concrete()) continue; // nothing to substitute here
                
                // try to substitute
                if(term.lhs.is_symbolic()) {
                    auto it = known_values.find(term.lhs.get_symbolic().version);
                    if(it != known_values.end() && it->first != term.lhs.get_symbolic().version) {
                        term.lhs = value(std::visit([](auto&& arg) -> long {
                ...
                if(term.rhs.is_symbolic()) {
                    auto it = known_values.find(term.rhs.get_symbolic().version);
                    if(it != known_values.end() && it->first != term.rhs.get_symbolic().version) {
                        term.rhs = value(std::visit([](auto&& arg) -> long {
                            return static_cast<long>(arg);
                        }, it->second));
                ...
                // if both sides are concrete, evaluate
                if(term.is_concrete()) {
                    bool satisfied = false;

                    switch(term.op) {
                        case EQ_EXPR: satisfied = term.lhs == term.rhs; break;
                        case NE_EXPR: satisfied = term.lhs != term.rhs; break;
                        case LT_EXPR: satisfied = term.lhs < term.rhs;  break;
                        case GT_EXPR: satisfied = term.lhs > term.rhs;  break;
                        case LE_EXPR: satisfied = term.lhs <= term.rhs; break;
                        case GE_EXPR: satisfied = term.lhs >= term.rhs; break;
                        default: break;
                    }

                    if(!satisfied) {
                        inner.unsatisfiable = true;
                        break;
                    }
                }
            }
